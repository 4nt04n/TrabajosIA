...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        # Crea una nueva sentencia con la celda y su recuento
        new_sentence = Sentence(self.get_neighbors(cell), count)

        # Agrega la nueva sentencia a la base de conocimiento
        self.knowledge.append(new_sentence)

        # Actualiza la base de conocimiento con nuevas inferencias
        self.update_knowledge()

    def get_neighbors(self, cell):
        """
        Returns the neighboring cells of the given cell.
        """
        i, j = cell
        neighbors = []
    ...
    def update_knowledge(self):
        """
        Update knowledge base with new inferences and propagate knowledge.
        """
        for sentence in self.knowledge:
            known_mines = sentence.known_mines()
            known_safes = sentence.known_safes()

            # Marca las minas y celdas seguras conocidas
            for mine in known_mines:
                self.mark_mine(mine)
            for safe in known_safes:
                self.mark_safe(safe)

           # Elimina sentencias vacías
            if not sentence.cells:
                self.knowledge.remove(sentence)

            # Realiza una comprobación de subconjunto e inferencias
